...
class SigningService:
    ...
    def _get_approver_ids(self, user_id: str) -> List[str]:
        """Return the user_id plus any users who delegated approval to them."""
        ...
        return [user_id] + delegating_ids

    def get_pending_signatures(self, user_id: str) -> List[Dict[str, Any]]:
        """Return Interview__c AND InteractionSummary records where the current
        user is assigned as CaseManager, PeerSupport, or Manager approver and
        has not yet signed.  Mirrors PendingDocumentationController."""
        try:
            logger.info(f"Fetching pending signatures for user {user_id}")
            all_approver_ids = self._get_approver_ids(user_id)
            ids_csv = ", ".join(f"'{uid}'" for uid in all_approver_ids)

            items: List[Dict[str, Any]] = []

            # ── 1) Interview__c (Treatment Plans + other Interview forms) ─────
            interview_query = f"""
            ...
            result = self.sf_client.query(interview_query)
            ...
            # ── 2) InteractionSummary (Clinical/Case/Peer Notes, CCA, etc.) ──
            # Skip records that have Interview__c set (those appear via the
            # Interview__c query above — same pattern as SF's getPendingManagerApprovals)
            interaction_query = f"""
            SELECT Id, Name, InteractionPurpose, Status,
                   Date_of_Interaction__c,
                   RelatedRecordId, Account.Name,
                   Requires_Manager_Approval__c, Manager_Approver__c,
                   Manager_Signed__c, Manager_Rejected__c,
                   CreatedBy.Name,
                   Interview__c
            FROM InteractionSummary
            WHERE Requires_Manager_Approval__c = true
              AND (Manager_Signed__c = false OR Manager_Signed__c = null)
              AND (Manager_Rejected__c = false OR Manager_Rejected__c = null)
              AND Manager_Approver__c IN ({ids_csv})
            ...
            is_result = self.sf_client.query(interaction_query)
            for r in is_result.get("records", []):
                client_name = (r.get("Account") or {}).get("Name")
                items.append({
                    "recordId": r.get("Id"),
                    "recordType": "Interaction",
                    "interviewId": None,
                    "interviewName": None,
                    "templateName": r.get("InteractionPurpose") or r.get("Name"),
                    "status": r.get("Status"),
                    "startedOn": r.get("Date_of_Interaction__c"),
                    "caseId": r.get("RelatedRecordId"),
                    "caseNumber": None,
                    "clientName": client_name,
                    "interactionSummaryId": r.get("Id"),
                    "pendingRoles": ["Manager"],
                    "caseManagerName": None,
                    "peerSupportName": None,
                    "managerName": None,
                    "createdByName": (r.get("CreatedBy") or {}).get("Name"),
                })
    ...
    def cosign_interview(
        ...
        try:
            ...
            if role == "CaseManager":
                ...
                all_approver_ids = self._get_approver_ids(user_id)
                if rec.get("Manager_Approver__c") not in all_approver_ids:
    ...
    def _maybe_advance_status(self, interview_id: str):
        """If all required signatures are collected, advance to Signed."""
        try:
            rec = self.sf_client.query(
            ...
            if cm_done and ps_done and mgr_done and r.get("Status__c") not in ("Signed", "Voided"):
                now_iso = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.000+0000")
                self.sf_client.update("Interview__c", interview_id, {
                    "Status__c": "Signed",
                    "Completed_On__c": now_iso,
                })
                logger.info(f"All signatures collected, advanced {interview_id} to Signed")

                # Also complete the linked InteractionSummary if present
                is_id = r.get("Interaction_Summary__c")
                if is_id:
                    try:
                        self.sf_client.update("InteractionSummary", is_id, {
                            "Status": "Completed",
                            "Completed_On__c": now_iso,
                        })
                    except Exception as e:
                        logger.warning(f"Could not update linked InteractionSummary {is_id}: {e}")
        except Exception as e:
            logger.warning(f"Could not advance interview status {interview_id}: {e}")

    def approve_interaction_summary(
        self,
        interaction_id: str,
        user_id: str,
    ) -> Dict[str, Any]:
        """Manager-approve an InteractionSummary (Clinical/Case/Peer Note, CCA, etc.).

        Mirrors SF's PendingDocumentationController.approveNote() for InteractionSummary.
        Sets Manager_Signed__c = true, Manager_Signed_Date__c, Status = Completed.
        """
        try:
            logger.info(f"Manager approve InteractionSummary {interaction_id} by user {user_id}")

            rec = self.sf_client.query(
                "SELECT Id, Manager_Approver__c, Manager_Signed__c, "
                "Requires_Manager_Approval__c, Status "
                "FROM InteractionSummary WHERE Id = :isId LIMIT 1",
                {"isId": interaction_id},
            ).get("records", [])
            if not rec:
                raise ValueError("InteractionSummary not found")
            rec = rec[0]

            if not rec.get("Requires_Manager_Approval__c"):
                raise ValueError("This record does not require manager approval")
            ...
            now_iso = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.000+0000")
            self.sf_client.update("InteractionSummary", interaction_id, {
                "Manager_Signed__c": True,
                "Manager_Signed_Date__c": now_iso,
                "Status": "Completed",
                "Completed_On__c": now_iso,
            })

            logger.info(f"Manager approved InteractionSummary {interaction_id}")
            return {"success": True, "message": "Approved as Manager"}
        except (ValueError, PermissionError):
            raise
        except Exception as e:
            logger.error(f"Manager approval failed for InteractionSummary {interaction_id}: {e}", exc_info=True)
            raise
